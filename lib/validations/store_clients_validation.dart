...
class StoreClientsValidation {
  static Response? validateRead(Request request) {
    ...
    return ValidationUtils.validateRequiredInt(request, 'id', 'Store client id is required');
  }
  ...
  static Response? _validateCreateOrUpdate(Request request, {required bool requireId}) {
    ...
    if (requireId) {
      final idError = ValidationUtils.validateRequiredInt(request, 'id', 'Store client id is required');
      if (idError != null) {
        return idError;
      }
    }
    ...
    final clientUuidError = ValidationUtils.validateRequiredUuid(request, 'clientUuid', 'Client uuid reference must be a valid UUID');
