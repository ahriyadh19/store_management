...
class ProductsValidation {
  static Response? validateRead(Request request) {
    ...
    return ValidationUtils.validateRequiredInt(request, 'id', 'Product id is required');
  }
  ...
  static Response? _validateCreateOrUpdate(Request request, {required bool requireId}) {
    ...
    if (requireId) {
      final idError = ValidationUtils.validateRequiredInt(request, 'id', 'Product id is required');
      if (idError != null) {
        return idError;
      }
    }

    final nameError = ValidationUtils.validateRequiredString(request, 'name', 'Product name is required');
    if (nameError != null) {
      return nameError;
    }

    final descriptionError = ValidationUtils.validateRequiredString(request, 'description', 'Product description is required');
