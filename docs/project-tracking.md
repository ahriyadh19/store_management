...
| 2026-05-20 | UI-008 | Realigned inventory-owned table permissions and hid inaccessible inventory sub-tabs/groups from the refactored module shell. | Copilot | Inventory section routing now matches the UI architecture, wildcard denies respect explicit blocks, and dead-end relation tabs are filtered out. |
| 2026-05-20 | CORE-003 | Centralized page route, permission, grouping, and module metadata into a shared catalog used by navigation, registry, ACL, and access-page samples. | Copilot | This removes the last major page-definition drift between the UI shell and permission catalog. |
...
| 4 | ACL-006 | Permission Audit Trail | Pending | Medium | Auditability is the next meaningful permission-system improvement after snapshot freshness is stabilized. |
| 5 | ACL-007 | Bulk Permission Templates | Pending | Medium | Shared templates are the next UX improvement for safe permission provisioning after catalog drift is removed. |
...
| UI-008 | Inventory Permission Routing Cleanup | Realigned refactored inventory tables with the inventory module permission boundary and filtered unauthorized inventory tabs/groups from the page shell. | Completed | High | Copilot | 2026-05-20 | 2026-05-20 | 2026-05-20 | `lib/services/access_control_service.dart`, `lib/views/pages/inventory_page.dart`, `test/services/access_control_service_test.dart`, `test/views/inventory_page_purchase_receipt_test.dart`, `test/views/model_module_pages_test.dart` | Inventory-owned ERP tables now inherit `page.inventory.view` for read fallback, and relation groups hide tabs that would only render unauthorized cards. |
| CORE-003 | Route Metadata Standardization | Centralized page route keys, permission keys, drawer grouping, tab grouping, icons, and module copy into one shared page catalog consumed by the UI shell and permission system. | Completed | Medium | Copilot | 2026-05-20 | 2026-05-20 | 2026-05-20 | `lib/views/index/index_page.dart`, `lib/views/index/index_page_registry.dart`, `lib/index.dart`, `lib/services/access_control_service.dart`, `lib/views/pages/model_module_pages.dart`, `test/services/access_control_service_test.dart`, `test/views/model_module_pages_test.dart` | Access-page sample data and ACL page permissions now resolve from the same source as navigation metadata. |
